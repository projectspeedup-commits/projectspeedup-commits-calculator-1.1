...
import { Cloud, LogOut, Plus, Trash2, Settings, Moon, Sun, Info, TrendingUp, Calculator, Wallet } from "lucide-react";
...
export function AdminPanel({
  ...
  const [activeTab, setActiveTab] = useState<"prices" | "economy">("prices");
                      ...
                      <table className="w-full text-left min-w-[500px]">
                        <thead className="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                          <tr>
                            ...
                            <th className="px-6 py-4 text-right">Норма на тн выпуска продукции (руб)</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {directItems.map(item => (
                            <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                <div className="relative w-full max-w-[192px] ml-auto">
                                  <input
                                    type="text"
                                    inputMode="decimal"
                                    value={item.norm}
                                    onChange={(e) => handleEconomyChange(item.id, 'norm', e.target.value)}
                                    className="w-full bg-slate-50 dark:bg-slate-800/50 border-0 rounded-xl pl-4 pr-10 h-11 text-right text-sm font-bold focus:ring-2 focus:ring-slate-400 dark:text-white"
                                  />
                                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 font-bold text-xs pointer-events-none">₽</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                ...
                <div className="bg-white dark:bg-[#1A1C19] rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                      <thead className="bg-slate-50/50 dark:bg-slate-800/50 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                        <tr>
                          <th className="px-6 py-4">Марка стали</th>
                          <th className="px-6 py-4 text-center">Политика (Круг) <RemnantPricingTooltip /></th>
                          <th className="px-6 py-4 text-center">Политика (Ш-гр) <RemnantPricingTooltip /></th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {allGrades.map(grade => {
                          ...
                          return (
                            <tr key={grade} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                              <td className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white">{grade}</td>
                              <td className="px-6 py-4">
                                <select
                                  value={pricing.round}
                                  onChange={(e) => handlePricingChange(grade, "round", e.target.value)}
                                  className={`bg-slate-50 dark:bg-slate-800 text-xs font-bold rounded-xl px-4 py-2.5 outline-none appearance-none cursor-pointer w-[160px] mx-auto block border-0 focus:ring-2 focus:ring-slate-400 ${
